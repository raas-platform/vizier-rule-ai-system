...
from app.models.rule import Rule, RuleAction, RuleCondition, ConditionTree
...
class TestRuleAnalyzer(unittest.TestCase):
    ...
    def test_3_invalid_operator(self):
        ...
        # 잘못된 연산자 이슈가 있는지 확인
        invalid_op_issues = [i for i in result.issues if i.issue_type == "invalid_operator"]
    ...
    def test_4_self_contradiction(self):
        ...
        self.assertGreater(len(intersection), 0, f"논리적 오류가 감지되어야 합니다. 감지된 타입: {issue_types}")
    ...
    def test_5_missing_condition(self):
        ...
        # 조건 누락 이슈가 있는지 확인
        missing_issues = [i for i in result.issues if i.issue_type == "missing_condition"]
    ...
    def test_field_analysis(self):
        ...
        age_analysis = next((fa for fa in result.field_analysis if fa.field_name == "age"), None)
    ...
    def test_cache_functionality(self):
        ...
        logger.info(f"첫 번째 실행: {first_time:.4f}초, 두 번째 실행: {second_time:.4f}초")
...
class TestRuleAnalyzerIntegration(unittest.TestCase):
    ...
    def test_complete_workflow(self):
        ...
        conditions = [
            ...
            RuleCondition(keyName="age", operator=">=", value=18, fieldDataType="number"),
            # 타입 불일치
            RuleCondition(keyName="score", operator=">=", value="invalid", fieldDataType="number"),
            # 잘못된 연산자
            RuleCondition(keyName="name", operator=">", value="John", fieldDataType="string"),
            # 자기모순
            RuleCondition(keyName="status", operator="==", value="active", fieldDataType="string"),
            RuleCondition(keyName="status", operator="!=", value="active", fieldDataType="string"),
        ]
        ...
        self.assertFalse(result.is_valid)  # 타입 불일치, 잘못된 연산자, 자기모순 등으로 인해
        self.assertGreater(len(result.issues), 0)
        ...
        # 각 이슈 타입이 존재하는지 확인
        issue_types = {issue.issue_type for issue in result.issues}
        expected_types = {"duplicate_condition", "type_mismatch", "invalid_operator", "self_contradiction"}
        
        # 적어도 일부 예상 타입이 존재해야 함
        intersection = issue_types.intersection(expected_types)
        self.assertGreater(len(intersection), 0, f"예상 이슈 타입 중 하나 이상이 감지되어야 합니다. 감지된 타입: {issue_types}")
