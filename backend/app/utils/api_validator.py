        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    result = (
                        True,
                        f"유효한 OpenAI API 키 (모델: {len(model_names)}개 사용 가능)",
                    )
                    ...
                    logger.info(
                        f"OpenAI API 키 검증 성공: {len(model_names)}개 모델 사용 가능"
                    )
                    return result
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    result = (
                        True,
                        f"유효한 Google AI API 키 (모델: {len(models)}개 사용 가능)",
                    )
                    ...
                    logger.info(
                        f"Google AI API 키 검증 성공: {len(models)}개 모델 사용 가능"
                    )
                    return result
        ...
        for i, (service_name, _) in enumerate(tasks):
            if isinstance(completed_tasks[i], Exception):
                results[service_name] = (
                    False,
                    f"검증 중 예외 발생: {str(completed_tasks[i])}",
                )
            else:
...
async def validate_api_keys_on_startup():
    ...
    try:
        ...
        if summary["all_valid"]:
            logger.info(
                f"✅ 모든 API 키 검증 성공 ({summary['valid_keys']}/{summary['total_keys']})"
            )
        else:
            logger.warning(
                f"⚠️ API 키 검증 완료: {summary['valid_keys']}/{summary['total_keys']} 유효"
            )
...
async def get_api_key_status() -> Dict[str, any]:
    ...
    return {
        ...
        "summary": summary,
        "services": {
            service: {"valid": valid, "message": message, "last_checked": "just_now"}
            for service, (valid, message) in results.items()
        },
    }
