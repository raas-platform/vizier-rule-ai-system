...
class TestRuleAnalysisConsistency(unittest.TestCase):
    ...
    def test_location_format_consistency(self):
        ...
        for issue in validation_result.issues:
            ...
            if issue.location:
                ...
                if (
                    issue.location == "전체 룰"
                    or issue.location == "전체 룰 구조"
                    or "필드 " in issue.location
                ):
                    continue
    ...
    def test_report_consistency_with_analysis(self):
        ...
        for issue_type, count in validation_result.issue_counts.items():
            # 디버깅 정보 출력
            logger.info(f"검사 중: {issue_type} (카운트: {count})")
    ...
    def test_report_vs_analyzer_sync(self):
        ...
        for issue_type in critical_issues:
            if (
                ...
                self.assertTrue(
                    found,
                    f"중요 이슈 타입 '{issue_type}'이 리포트에 반영되지 않았습니다.",
                )
    ...
    async def test_analyzer_integration(self):
        """분석기 통합 테스트"""
        analyzer = RuleAnalyzerV2()

    async def test_analyzer_performance_batch(self):
        """배치 분석 성능 테스트"""
        analyzer = RuleAnalyzerV2()

    async def test_analyzer_edge_cases(self):
        """극단적인 케이스 테스트"""
        analyzer = RuleAnalyzerV2()
...
async def test_issue_count_consistency(rule_with_multiple_issues):
    ...
    assert (
        result.issue_counts == manual_counts
    ), f"자동 카운트 {result.issue_counts}와 수동 카운트 {manual_counts}가 일치해야 함"

    # 총 이슈 개수 확인
    total_count_from_issues = len(result.issues)
    total_count_from_counts = sum(result.issue_counts.values())
    assert (
        total_count_from_issues == total_count_from_counts
    ), f"이슈 배열 길이({total_count_from_issues})와 카운트 합계({total_count_from_counts})가 일치해야 함"

    # 요약 메시지에 총 이슈 개수와 유형 개수가 모두 포함되었는지 확인
    assert (
        str(len(result.issue_counts)) in result.summary
    ), f"요약에 이슈 유형 개수({len(result.issue_counts)})가 포함되어야 함"
    assert (
        str(total_count_from_issues) in result.summary
    ), f"요약에 총 이슈 개수({total_count_from_issues})가 포함되어야 함"
...
async def test_report_issues_consistency(rule_with_multiple_issues):
    ...
    report = await report_service.generate_report(
        rule_with_multiple_issues, validation_result
    )
    ...
    assert (
        str(total_issues) in report_content
    ), f"리포트에 총 이슈 건수({total_issues})가 포함되어야 함"
...
async def test_duplicate_condition_counts():
    ...
    assert (
        "duplicate_condition" in result.issue_counts
    ), "중복 조건 이슈가 감지되어야 함"
    ...
    duplicate_issues = [
        i for i in result.issues if i.issue_type == "duplicate_condition"
    ]
    assert (
        len(duplicate_issues) == duplicate_count
    ), f"중복 조건 이슈 개수({len(duplicate_issues)})가 카운트({duplicate_count})와 일치해야 함"
