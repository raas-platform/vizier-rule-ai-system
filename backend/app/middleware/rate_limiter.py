...
class RateLimiter:
    ...
    def __init__(self):
        ...
        logger.info(f"Rate Limiter 초기화: {self.rate_limit_per_minute}/분, {self.rate_limit_per_hour}/시간, {self.rate_limit_per_day}/일")
    ...
    def _cleanup_old_requests(self):
        ...
        if keys_to_remove:
            logger.debug(f"Rate Limiter 정리: {len(keys_to_remove)}개 클라이언트 기록 제거")
    ...
    def _get_rate_limit_status(self, client_key: str) -> Dict[str, int]:
        ...
        requests_last_minute = sum(1 for req_time in request_times if req_time > minute_ago)
    ...
    def is_allowed(self, request: Request) -> tuple[bool, Dict[str, any]]:
        ...
        # 응답 헤더 정보
        return True, {
            "remaining_minute": self.rate_limit_per_minute - status["requests_last_minute"] - 1,
            "remaining_hour": self.rate_limit_per_hour - status["requests_last_hour"] - 1,
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if not is_allowed:
        logger.warning(f"Rate limit 초과: {rate_limiter._get_client_key(request)} - {limit_info['message']}")
    ...
    response.headers["X-RateLimit-Limit-Minute"] = str(rate_limiter.rate_limit_per_minute)
