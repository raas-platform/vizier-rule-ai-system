...
async def test_modular_architecture():
    ...
    try:
        ...
        conditions = analyzer.condition_analyzer.parse_conditions(
            test_rule.conditionTree
        )
        ...
        structure_metrics = analyzer.structure_analyzer.analyze_structure(
            test_rule.conditionTree
        )
        logger.info(
            f"   ✅ 구조 메트릭: 깊이={structure_metrics['depth']}, 복잡성={structure_metrics['complexity_score']}"
        )
    ...
    # 5. IssueDetector 테스트
    try:
        issues = analyzer.issue_detector.detect_all_issues(
            test_rule, conditions, field_types
        )
        logger.info(f"   ✅ IssueDetector: {len(issues)}개 이슈 검출")

        # ReportGenerator 테스트
        optimized_issues = analyzer.report_generator.optimize_issues(issues)
        rule_summary = analyzer.report_generator.generate_rule_summary(
            test_rule, optimized_issues
        )
        logger.info(
            f"   ✅ ReportGenerator: 이슈 최적화 ({len(issues)} → {len(optimized_issues)})"
        )
    ...
    try:
        ...
        logger.info(
            f"   ✅ 성능 메트릭: {'✅' if result.performance_metrics else '❌'}"
        )
