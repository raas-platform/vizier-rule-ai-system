    ...
    try:
        ...
        for i, condition in enumerate(rule.conditionTree.condition):
            if hasattr(condition, 'logicType'):
                logger.info(f"  조건 {i+1}: {condition.logicType} 그룹 ({len(condition.condition)}개 하위 조건)")
            else:
                logger.info(f"  조건 {i+1}: {condition.keyName} {condition.operator} {condition.value} ({condition.fieldDataType})")
