...
> **1차 개발 완료 (2025-06)** – 본 문서는 LLM 호출 및 데이터 흐름을 강조하는 *Functional-Architecture* 버전으로 업데이트되었습니다.
...
| Layer | Main Library / Tool | Version / Notes |
|-------|--------------------|-----------------|
| **Backend API** | FastAPI | 0.104.1 (ASGI) |
| **Python 런타임** | CPython | 3.12 (dev) / 3.11 (prod) |
| **Data-Model** | Pydantic | 2.4.2 |
| **Async I/O** | httpx / anyio | 비동기 LLM API 호출 |
| **AI SDK** | openai · anthropic · google-generativeai | 멀티 provider 지원 |
| **DB** | SQLite → PostgreSQL | SQLAlchemy 2.x |
| **CI / CD** | GitHub Actions + Docker | Blue-Green 배포 |
| **Test** | pytest · pytest-cov | 커버리지 ≈ 82 % |

> LLM 호출량과 토큰 사용량 모니터링을 위해 **prometheus_exporter** 플러그인이 추가 예정입니다.
...
graph TD
  subgraph Client GUI
    G1["Vue/React<br/>Rule Editor"]
  end
  subgraph API Layer
    A1["POST /rules/validate-json"]
    A2["POST /rules/generate-ai-html-report"]
  end
  subgraph Analysis Engine
    B1["RuleAnalyzerV2"]
    B2["AIEnhancer<br/>+ LLM Service"]
  end
  subgraph LLM Providers
    C1[("GPT-4o / Claude-3 / Gemini 1.5")] 
  end
  G1 -- 룰 JSON --> A1 --> B1
  B1 -- prompt --> B2 -- HTTPS --> C1
  C1 -- answer --> B2 -- enrich --> B1
  B1 -- result JSON --> A1 --> G1
  G1 -- validation JSON --> A2 --> B2 --> C1 --> B2 --> A2 --> G1
```

*데이터 흐름은 실선, 컴포넌트 경계는 subgraph로 표시했습니다.*
...
-> **2025-06-18 기준** – `pytest` **5 pass / 0 fail / 0 error**  
-> 테스트 커버리지는 샘플 환경 기준 **~80 %** (추정) 